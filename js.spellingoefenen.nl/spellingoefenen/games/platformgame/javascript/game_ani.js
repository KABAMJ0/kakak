...
function drawAll()
{
	...
	else if(game["status"] == "")
	{
		...
		background = game["level"]%7 + 1;
	
		
		drawImage(manifest["background"+background], spot["BG"].x-(game["Camera-X"]/10), spot["BG"].y);
		...
		if (game["gameover"]) 
		{
			drawText("Game over!", "PLATFORMLEVEL_TEXT");
			if(game["DieTimer"]>30) {endGame();}
		
		}
		
		if(game["DieTimer"]>10){context.globalAlpha = ((game["DieTimer"]-10)/25);}
		...
		if(game["GoNextLevel"]>19){
			game["DieTimer"]=game["GoNextLevel"]-19;
			
			Templevel = game["level"] + 1;
			
			if(game["GoNextLevel"]>45 && Templevel != 10){
				context.globalAlpha = 1;
				drawText("Level: "+ Templevel, "PLATFORMLEVEL_TEXT");
			}
			else if(game["GoNextLevel"]>35 && Templevel == 10){
			
			context.globalAlpha = 1;
			drawText("Gefeliciteert, je hebt alle levels gehaald!! Je krijgt 500 bonuspunten!!", "PLATFORMLEVEL_TEXT");
				
			}
			
			if(game["GoNextLevel"]>60){game["level"]+=1;StartLevel(game["level"])}
		...
		drawImage(manifest["site_background"],880,0);
		
		drawImage(manifest["key_outline"],1170,300)
		if(game["keyfound"]==true){drawImage(manifest["key"],1170,300)}
		
		drawText("LEVEL: " + game["level"], "PLATFORM_LEVEL");
		drawText("SCORE: " + game["score"], "PLATFORM_SCORE");
		drawText("LEVELSCORE: " + game["levelscore"] + " (" + "25" + ")" , "PLATFORM_LEVELSCORE");
		
		drawText(game["spelword"], "PLATFORM_SPELWORD");
		renderParticles("");
		Tempspelledword = game["spelledword"] + "_";
		drawText(Tempspelledword, "PLATFORM_SPELLEDWORD");
		
		
		for(i = 1; i <= game["lives"]; i++)
		{
			//if(game["lives"] >= i) thisHeart = "heart"; else thisHeart = "heart_empty";
			
			drawImage(manifest["idle1"], 1075 + (i * 50), 620,55,65);
		}	

		
		context.globalAlpha = game["TextTimer"] * 0.033;
		if (game["TextTimer"]>0){game["TextTimer"]-=0.50;}
		
		
		drawText("Je kunt nu de sleutel vinden!", "PLATFORM_TEXT");
	...
	{	
		...
		if(hoverSpot("MUSIC_ICON")) context.globalAlpha = 1; else context.globalAlpha = 0.75;
	
			spot["MUSIC_ICON"].x = temp_x;
			if (game["music"]) icon = "music_on"; else icon = "music_off"
			drawImage(manifest[icon], spot["MUSIC_ICON"].x, spot["MUSIC_ICON"].y);
			temp_x -= spot["MUSIC_ICON"].width + spot["WINDOW_BUTTONS"].margin;
