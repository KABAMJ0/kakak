...
function animateAll()
{
	if(gameEngine["play"])
	{
		...
		{
			game["hangmanFireFrame"]++;
			if(game["hangmanFireFrame"] > 16) game["hangmanFireFrame"] = 1;
		
			
			if(game["status"] == "") hangmanAni();
...
function drawAll()
{
	...
	if(game["status"] == "PLAYBUTTON" || game["status"] == "PRELOAD" || game["status"] == "INTRO")
	{
		...
		//if(game["hangmanEndedY"] < 2) 
		
		game["hangmanEndedY"] = 0;
		//game["hangmanEndedY"] *= 1.25;
		...
		context.globalAlpha = 1;
		drawImage(manifest["logo"], spot["INTRO_LOGO"].x, spot["INTRO_LOGO"].y, true, true, 0, false, false, true);
	...
	else if(game["status"] == "HIGHSCORES")
	{
		...
		if(game["hangmanEndedY"] < 2) game["hangmanEndedY"] = 2;
		game["hangmanEndedY"] *= 1.25;
